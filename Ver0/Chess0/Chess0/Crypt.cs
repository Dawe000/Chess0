...
namespace Chess0
{
    public class Crypt
    {
        ...
        static void InitBlowFish(string Hash) //initialise variables for blowfish algorithm
        {
            S = SBox.Get();
        ...
        public string FullEncrypt(Item PlainItem)
        {
            string plainText = PlainItem.siteUrl + "^" + PlainItem.passWord + "^" + PlainItem.note;
        ...
        public Item FullDecrypt(string CypherText)
        {
            string plainHex = "";
            string tempHold = "";
            ...
            string[] plainList = new string[3];
            int count = 0;
            int i = 0;
            while (count < 3)
            {
                if (plainText[i] == '^') count++;
                else plainList[count] += plainText[i];
                i++;
            }
            return new Item(new string[] { "", plainList[0], plainList[1], plainList[2] });
        }
        uint RotateLeft(this uint value, int count) //needed for algorithms
        {
